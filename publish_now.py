#!/usr/bin/env python3
"""Flip an already-uploaded scheduled/private YouTube video to public now.
...
but you want to release it immediately instead of waiting for its slot.
    ...
    python publish_now.py <video_id>
"""
...
from youtube_automation.youtube_uploader import set_video_public


def main() -> None:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("video_id", help="The YouTube video ID to publish now.")
    ...
    status = set_video_public(args.video_id, config)
