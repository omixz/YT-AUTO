...
def _compose_scene(rng, scene: Scene, config: PipelineConfig, subject: Optional[str], forced_setting: Optional[str] = None):
    ...
    if setting in _INTERIOR_VARIANTS:
        ...
        base_color = GROUND_COLORS.get(setting, GROUND_COLORS["default"])
        jitter = rng.randint(-14, 14)
        ground_color = tuple(max(0, min(255, c + jitter)) for c in base_color)
...
def generate_scene_image(
    ...
    base, paint, _motion = _compose_scene(rng, scene, config, subject, forced_setting=forced_setting)
...
def generate_scene_clip(
    ...
    w, h = config.video.resolution
    subject = _resolve_subject(scene, index, subject_fallback)
    base, paint, (ax, ay, px, py) = _compose_scene(rng, scene, config, subject, forced_setting=forced_setting)
    ...
    if subject in _AQUATIC_SUBJECTS or subject is None:
        ...
        frame_pattern = _build_sprite_frames(
            rng, paint, w, h, work_dir, index, bubbles=subject in _AQUATIC_SUBJECTS, loop_frames=loop_frames,
        )
